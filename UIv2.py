...
def run_button_pressed():
    game=combobox_1.get()
    act_distance=int(act_range.get())
    mouse_speed=round(speed_range.get(),2)
    x=int(size_x_range.get())
    y=int(size_y_range.get())
    body_multiplier=((int(crosshair_range.get())-80)*-0.8)/530+0.9 #lineáris interpoláció a csúszka pozíciójából leszámolva
    #print("Játék: {} aktivciós: {} egér: {} x:{} y: {} body: {}".format(game,act_distance,mouse_speed,x,y,body_multiplier))
    new_thread=multiprocessing.Process(target=Aimbot,args=(game,act_distance,mouse_speed,x,y,body_multiplier))
    global stop
    if(stop==False):
        new_thread.start()
...
size_x_range=ctk.CTkSlider(master=bar_frames,width=300, from_= 640, to=1920, command=x_slide,number_of_steps=1200, progress_color="#e40d11")
size_x_range.grid(row=1, column=0, sticky="NW",padx=130,pady=345)
size_x_range.set(1024)
...
size_y_range=ctk.CTkSlider(master=bar_frames,width=300, from_= 480, to=1080, command=y_slide,number_of_steps=600, progress_color="#e40d11")
size_y_range.grid(row=1, column=0, sticky="NW",padx=130,pady=405)
size_y_range.set(768)
size_x.set(str(int(size_x_range.get())))
size_y.set(str(int(size_y_range.get())))
